...
small (Config ({ options } as c)) =
    Config { c | options = { options | modalSize = Just SM } }


{-| Option to make a modal larger than the default
-}
large : Config msg -> Config msg
large (Config ({ options } as c)) =
    Config { c | options = { options | modalSize = Just LG } }
...
hideOnBackdropClick hide (Config ({ options } as c)) =
    Config { c | options = { options | hideOnBackdropClick = hide } }
...
withAnimation animateMsg (Config c) =
    Config { c | withAnimation = Just animateMsg }
...
view :
    ...
    -> Config msg
    -> Html.Html msg
view visibility (Config c as ca) =
    Html.div
        []
        ([ Html.div
            ([ Attr.tabindex -1] ++ display visibility c)
            [ Html.div
                ([ Attr.attribute "role" "document"
                 , Attr.class "elm-bootstrap-modal"
                 ]
                    ++ modalAttributes c.options
                    ++ (if c.options.hideOnBackdropClick then
                            [ Events.on "click" (containerClickDecoder c.closeMsg) ]
                        else
                            []
                       )
                )
...
display visibility c =
    case visibility of
        ...
        FadeClose ->
            ...
            , Events.on "transitionend" (Json.succeed c.closeMsg)
            ]
...
isFade c =
    Maybe.map (\_ -> True) c.withAnimation |> Maybe.withDefault False
...
header :
    ...
    -> Config msg
    -> Config msg
header attributes children (Config c) =
    Config
        { c
            | header =
                Just <|
...
body :
    ...
    -> Config msg
    -> Config msg
body attributes children (Config c) =
    { c
        | body =
            Just <|
                Body
...
footer :
    ...
    -> Config msg
    -> Config msg
footer attributes children (Config c) =
    { c
        | footer =
            Just <|
                Footer
    ...
    case c.header of
        Just (Header cfg) ->
            Html.div
                ...
                (cfg.children ++ [ closeButton <| getCloseMsg c ])
                |> Just
